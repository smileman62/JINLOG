...
description: "폴더 구조 & 아키텍처 프론트엔드 프로젝트가 작을 때는 보통 components, hooks, utils, api, pages 정도로 폴더를 나눠도 큰 문제가 없습니다. 하지만 기능이 많아지고, 로그인, 게시글, 댓글, 마이페이지, 관리자 기능,…"
date: "2026-05-17"
tags: ["react"]
published: true
---
...
import type { Anime } from '../model/types';
...
export function AnimeCard({ anime, rightSlot }: AnimeCardProps) {
  return (
...
```ts
import { AnimeCard, type Anime } from '@/entities/anime';
```
...
interface SelectedAnimeState {
  selectedAnimeList: Anime[];
  selectAnime: (anime: Anime) => void;
  removeAnime: (animeId: number) => void;
}

export const useSelectedAnimeStore = create<SelectedAnimeState>((set) => ({
  selectedAnimeList: [],

  selectAnime: (anime) =>
    set((state) => {
      const alreadySelected = state.selectedAnimeList.some(
...
interface SelectAnimeButtonProps {
  anime: Anime;
}

export function SelectAnimeButton({ anime }: SelectAnimeButtonProps) {
  const selectAnime = useSelectedAnimeStore((state) => state.selectAnime);
      ...
      <div className="mt-4 grid grid-cols-2 gap-4 md:grid-cols-4">
        {animeList.map((anime) => (
          <AnimeCard
            key={anime.id}
            anime={anime}
            rightSlot={<SelectAnimeButton anime={anime} />}
          />
        ))}
      </div>
    </section>
...
export function AnimeSelectPage() {
  return (
    <main>
      <AnimeSearchPanel />
      <SelectedAnimeGrid />
